...
// //   const hasChanges = JSON.stringify(details) !== JSON.stringify(tempDetails) || profileImage !== details.image || coverImage !== details.cover_image;
...
// //         const userDetails = detailsData?.user || {};
...
// //         setCoverImage(userDetails.cover_image || null);
...
// //       formData.append("details", JSON.stringify(tempDetails));
// //       if (profileImage && profileImage !== details.image) formData.append("profileImage", profileImage);
// //       if (coverImage && coverImage !== details.cover_image) formData.append("coverImage", coverImage);
...
// //       if (data.success) {
// //         setDetails({ ...details, ...data.user });
// //         setTempDetails({ ...tempDetails });
// //         setIsModalOpen(false);
// //         Swal.fire({ position: "top-end", icon: "success", title: "Profile updated!", showConfirmButton: false, timer: 1500 });
// //       } else {
...
// //           <div className="relative group">
// //             <img
// //               src={profileImage ? (typeof profileImage === "string" ? profileImage : URL.createObjectURL(profileImage)) : "/defult_profile.jpg"}
// //               className="w-24 h-24 sm:w-32 sm:h-32 rounded-full border-4 border-white shadow-lg object-cover"
// //               alt="Profile"
// //             />
// //             <label className="absolute bottom-0 right-0 w-10 h-10 bg-black/50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition text-white cursor-pointer">
...
// //             <>
// //               <p className="text-gray-600 text-sm whitespace-pre-line">{bio}</p>
// //               <button onClick={() => setIsEditing(true)} className="w-full border border-[#0000FF] text-[#0000FF] rounded-lg mt-3 py-2 hover:bg-orange-50 transition">
// //                 Edit Bio
// //               </button>
// //             </>
// //           ) : (
...
// //               <div className="flex gap-2 mt-3">
// //                 <button onClick={handleCancelBio} className="w-1/2 border rounded-lg py-2 text-gray-600 hover:bg-gray-100">Cancel</button>
// //                 <button onClick={handleSaveBio} disabled={tempBio.trim() === bio.trim()} className={`w-1/2 rounded-lg py-2 text-white transition ${tempBio.trim() === bio.trim() ? "bg-gray-300 cursor-not-allowed" : "bg-[#0000FF] hover:bg-[#a9471c]"}`}>
// //                   Save
// //                 </button>
// //               </div>
// //             </>
...
// //           {/* DETAILS */}
// //           <div className="mt-6 space-y-3 text-sm text-gray-700">
// //             <div className="flex items-center gap-3"><Briefcase size={14} className="text-[#0000FF]" /><span>{details.work || "add work"}</span></div>
// //             <div className="flex items-center gap-3"><GraduationCap size={14} className="text-[#0000FF]" /><span>{details.education || "add education"}</span></div>
// //             <div className="flex items-center gap-3"><MapPin size={14} className="text-[#0000FF]" /><span>{details.location || "add location"}</span></div>
// //           </div>

// //           {details?.skills && (() => {
// //             const skillsArray = Array.isArray(details.skills) ? details.skills.map(s => s.trim()).filter(Boolean) : typeof details.skills === "string" ? details.skills.split(",").map(s => s.trim()).filter(Boolean) : [];
// //             return skillsArray.length > 0 ? (
// //               <div className="mt-5 border-t pt-4">
// //                 <h4 className="font-semibold text-gray-900 mb-2 text-sm">Skills</h4>
// //                 <div className="flex flex-wrap gap-2">
// //                   {skillsArray.map((skill, i) => <span key={i} className="px-3 py-1 text-xs bg-orange-50 text-[#0000FF] rounded-full border border-orange-200">{skill}</span>)}
// //                 </div>
// //               </div>
// //             ) : null;
// //           })()}
...
// //             <div className="flex items-center gap-3"><Globe size={14} className="text-[#0000FF]" /><a href={details.website || "#"} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">{details.website || "https://example.com"}</a></div>
...
// //           <button onClick={() => setIsModalOpen(true)} className="w-full mt-4 border bg-[#0000FF] text-white rounded-lg py-2 hover:bg-[#a9471c] transition">Edit Details</button>
// //         </div>

// //         {/* RIGHT: POSTS */}
// //         <div className="lg:col-span-8 space-y-6">
// //           <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
// //             <input type="text" placeholder="Search posts..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full sm:w-1/2 border border-orange-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#0000FF] outline-none" />
// //             <select value={sortOption} onChange={(e) => setSortOption(e.target.value)} className="w-full sm:w-1/4 border border-orange-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#0000FF] outline-none bg-white">
// //               <option value="default">🔁 Default</option>
// //               <option value="newest">🕒 Newest</option>
// //               <option value="oldest">📅 Oldest</option>
// //               <option value="title-asc">🔤 Title (A–Z)</option>
...
// //           {posts.length === 0 ? (
// //             <div className="text-center text-gray-500 bg-white p-10 rounded-xl shadow-sm">No posts found.</div>
// //           ) : (
// //             posts.map(post => (
// //               <article key={post._id} className="rounded-xl p-4 border border-gray-200 bg-white shadow-sm hover:shadow-md transition-all">
// //                 <div className="flex items-center gap-3">
// //                   <img className="w-10 h-10 rounded-full object-cover" src={session?.user?.image || "/defult_profile.jpg"} alt="Author" />
// //                   <div>
// //                     <p className="text-gray-900 font-medium">{session?.user?.name}</p>
// //                     <small className="text-gray-500 text-xs">{formatFacebookDate(post.created_at)}</small>
// //                   </div>
...
// //                 {post.featured_image && (
// //                   <img src={post.featured_image} alt={post.blog_title} className="w-full max-h-[400px] object-cover mt-3 rounded-lg" />
// //                 )}
...
// //                 <div className="flex justify-between items-center mt-3 text-sm text-gray-500">
// //                   <div className="flex items-center gap-2"><FaHeart className="text-red-500" /> {post.likes || 0} Likes</div>
// //                   <div>{post.comment || 0} Comments · {post.shares || 0} Shares</div>
// //                 </div>
// //               </article>
// //             ))
// //           )}
...
// import { useSession } from "next-auth/react";
// import { FaHeart } from "react-icons/fa";
// import { Briefcase, Camera, Globe, GraduationCap, Languages, Mail, MapPin } from "lucide-react";
// import { BsPostcard } from "react-icons/bs";
// import { SlUserFollowing } from "react-icons/sl";
// import { FiEdit, FiPhone } from "react-icons/fi";
// import PostField from "@/app/(main)/components/PostField/PostField";
...
//       try {
//         const [bioRes, postsRes, detailsRes] = await Promise.all([
//           fetch(`/api/edit-bio?email=${session.user.email}`),
//           fetch(`/api/get-user-posts?email=${session.user.email}`),
...
//       const formData = new FormData();
//       formData.append("email", session.user.email);
//       if (type === "profile") formData.append("profile", file);
//       if (type === "cover") formData.append("cover", file);

//       const res = await fetch("/api/update-user-profile", { method: "POST", body: formData });
//       const data = await res.json();

//       if (data.success) {
//         Swal.fire({
//           icon: "success",
//           title: `${type === "profile" ? "Profile" : "Cover"} updated!`,
//           showConfirmButton: false,
//           timer: 1500,
//         });
//         if (type === "profile") setProfileImage(URL.createObjectURL(file));
//         if (type === "cover") setCoverImage(URL.createObjectURL(file));
//       } else {
...
//         method: "PATCH",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({ email: session.user.email, bio: tempBio.trim() }),
//       });
//       const data = await res.json();
//       if (data.success) setBio(tempBio.trim());
//     } catch (err) {
...
//   const handleCancelBio = () => setTempBio(bio);

//   // Save details (without images)
//   const handleModalSave = async () => {
//     if (!session?.user?.email) return;
//     try {
//       const formData = new FormData();
//       formData.append("email", session.user.email);
//       Object.keys(tempDetails).forEach(key => formData.append(key, tempDetails[key]));
...
//           {coverImage && <img src={typeof coverImage === "string" ? coverImage : URL.createObjectURL(coverImage)} alt="Cover" className="w-full h-full object-cover" />}
//           <label className="absolute top-3 right-3 bg-white px-3 py-1 rounded-lg flex items-center gap-1 hover:bg-gray-200 cursor-pointer">
//             <Camera size={16} /> Change Cover
//             <input type="file" accept="image/*" onChange={(e) => handleUpload(e.target.files[0], "cover")} className="hidden" />
//           </label>
//         </div>
...
//             <img src={profileImage ? (typeof profileImage === "string" ? profileImage : URL.createObjectURL(profileImage)) : "/defult_profile.jpg"} className="w-32 h-32 rounded-full border-4 border-white object-cover" alt="Profile" />
//             <label className="absolute bottom-0 right-0 bg-black/50 text-white w-10 h-10 rounded-full flex items-center justify-center cursor-pointer">
//               <Camera size={16} />
//               <input type="file" accept="image/*" onChange={(e) => handleUpload(e.target.files[0], "profile")} className="hidden" />
//             </label>
//           </div>
//         </div>
...
//           <h1 className="text-xl font-bold">{session?.user?.name || "Anonymous User"}</h1>
//         </div>
//       </div>
...
//           ) : (
//             <>
//               <textarea className="w-full h-32 border border-orange-200 rounded-lg p-3 text-sm text-gray-700 focus:ring-2 focus:ring-[#0000FF]" value={tempBio} onChange={(e) => setTempBio(e.target.value)} />
//               <div className="flex gap-2 mt-3">
//                 <button onClick={handleCancelBio} className="w-1/2 border rounded-lg py-2 text-gray-600 hover:bg-gray-100">Cancel</button>
//                 <button onClick={handleSaveBio} className="w-1/2 rounded-lg py-2 text-white bg-[#0000FF] hover:bg-[#a9471c]">Save</button>
//               </div>
//             </>
//           )}
...
//         <div className="lg:col-span-8 space-y-6">
//           <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
//             <input type="text" placeholder="Search posts..." className="w-full sm:w-1/2 border border-orange-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#0000FF] outline-none" />
//           </div>
...
//                 {post.featured_image && <img src={post.featured_image} alt={post.blog_title} className="w-full max-h-[400px] object-cover mt-3 rounded-lg" />}
//                 <h2 className="text-lg font-semibold mt-3">{post.blog_title}</h2>
//                 <p className="text-gray-600 text-sm mt-1">{post.description}</p>
